...
const isHost = (row, id) => row && row.host_identity === id;
const isCohost = (row, id) => row && Array.isArray(row.cohosts) && row.cohosts.includes(id);
const canManage = (row, id) => isHost(row, id) || isCohost(row, id);
...
export async function POST(req) {
  ...
  if (action === 'setMode' || action === 'promote' || action === 'demote') {
    if (!isHost(host, identity)) return NextResponse.json({ error: 'Doar gazda principală poate face asta.' }, { status: 403 });
    if (action === 'setMode') {
  ...
  if (!canManage(host, identity)) return NextResponse.json({ error: 'Nu ai drepturi de gazdă.' }, { status: 403 });
