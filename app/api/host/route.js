...
export async function POST(req) {
  ...
  const { room, action, identity, name, target } = body;
  const supabase = getSupabase();
  if (!supabase) return NextResponse.json({ error: 'Baza de date nu e configurată.' }, { status: 400 });

  // Revendică rolul de gazdă (primul venit devine gazdă)
  if (action === 'claim') {
    ...
    if (!host) {
      ...
      host = { host_identity: identity };
    }
    return NextResponse.json({ host_identity: host.host_identity });
  }

  if (action === 'status') {
    const host = await getHost(supabase, room);
    return NextResponse.json({ host_identity: host?.host_identity || null });
  }

  // Acțiuni de gazdă — doar gazda le poate face
  const host = await getHost(supabase, room);
  ...
  try {
    if (action === 'muteAll') {
      ...
      for (const p of parts) {
        ...
        for (const t of p.tracks || []) {
          if (isAudio(t) && !t.muted) {
            try { await svc.mutePublishedTrack(room, p.identity, t.sid, true); } catch (e) {}
          }
        }
      }
    ...
    if (action === 'mute' && target) {
      ...
      if (p) {
        for (const t of p.tracks || []) {
          if (isAudio(t)) {
            try { await svc.mutePublishedTrack(room, target, t.sid, true); } catch (e) {}
          }
        }
      }
      return NextResponse.json({ ok: true });
