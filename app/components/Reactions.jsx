...
export default function Reactions() {
  ...
  const [hands, setHands] = useState({}); // id -> name
  const [myHand, setMyHand] = useState(false);

  const spawn = useCallback((emoji) => {
    const id = Math.random().toString(36).slice(2);
    const x = 6 + Math.random() * 30; // % from left
  ...
  const send = useCallback(
    (payload, reliable) => {
      ...
      try {
        localParticipant.publishData(enc.encode(JSON.stringify(payload)), {
          reliable,
          topic: 'reactions',
        });
      } catch (e) {}
  ...
  return (
    <>
      <div className="rx-floats">
        {floats.map((f) => (
          <span key={f.id} className="rx-float" style={{ left: `${f.x}%` }}>
            {f.emoji}
          </span>
        ))}
      </div>
      ...
      <div className="rx-stack">
        {handNames.length > 0 && (
          <div className="rx-hands">
            ✋ {handNames.slice(0, 3).join(', ')}
            {handNames.length > 3 ? ` +${handNames.length - 3}` : ''}
          </div>
        )}
        <div className="rx-bar">
          {EMOJIS.map((e) => (
            <button key={e} onClick={() => react(e)} className="rx-emoji">
              {e}
            </button>
          ))}
          <button className={`rx-hand ${myHand ? 'on' : ''}`} onClick={toggleHand} title="Ridică mâna">
            ✋
          </button>
        </div>
      </div>
